...
class SignalGenerator:
    ...
    async def scan_all(self) -> List[TradeSignal]:
        ...
        signals.sort(key=lambda s: (s.quality.value, s.risk_reward), reverse=True)
...
# Fix: need to reference entry_candles_parsed in methods
# We'll pass it as parameter or recompute. Let me fix the volume check.
# Actually, the methods need entry_candles_parsed. Let me add it as parameter.
# But for simplicity, I'll recompute volume from the last candle in entry_structure context.
# Let me just use the indicators.volume_sma[-1] and get current volume differently.
# Actually the volume is in the last candle. We need access to it.
# Let me update the analyze_symbol to pass current_volume.